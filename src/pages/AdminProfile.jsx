...
export default function AdminProfile() {
  ...
  useEffect(() => {
    ...
    window.addEventListener("admin-profile-updated", handleAdminDataChanged);
    ...
    return () => {
      window.removeEventListener("admin-profile-updated", handleAdminDataChanged);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const updated = { 
        ...
        avatar_url: dataForm.avatar_url 
      };
      localStorage.setItem("admin", JSON.stringify(updated));
      
      // trigger realtime sync for admin header
      window.dispatchEvent(new Event("admin-profile-updated"));

      setSuccess("Profil Admin sukses di-update! ✨");
  ...
  const displayAvatar = dataForm.avatar_url?.startsWith("http")
    ? dataForm.avatar_url
    : `https://ui-avatars.com/api/?name=${encodeURIComponent(dataForm.username || "Admin")}&background=FCE7F3&color=ED346C&bold=true`;

  if (loading && !dataForm.username) return <div className="py-20"><LoadingSpinner text="Sinkronisasi profil otorisasi..." /></div>;

  return (
    <div className="space-y-6 text-left p-4 bg-[#FFFBFB] min-h-screen">
      <PageHeader 
        title="Admin Profile Management" 
        subtitle="Perbarui identitas publik, tautan foto avatar, dan ganti kata sandi keamanan masuk Admin." 
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
        <div className="bg-white border border-pink-100 p-5 rounded-2xl shadow-sm text-center flex flex-col items-center">
          ...
          <h3 className="font-bold text-slate-800 text-sm tracking-tight">{dataForm.username || "Admin"}</h3>
